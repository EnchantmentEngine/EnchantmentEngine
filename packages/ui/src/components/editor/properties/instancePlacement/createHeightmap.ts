import { Entity, getComponent, getNestedChildren, hasComponent } from '@ir-engine/ecs'
import { ObjectComponent } from '@ir-engine/spatial/src/renderer/components/ObjectComponent'
import { VisibleComponent } from '@ir-engine/spatial/src/renderer/components/VisibleComponent'
import { Mesh, MeshDepthMaterial, OrthographicCamera, Scene, WebGLRenderer } from 'three'

function createHeightmap(rootEntities: Entity[], distance = 100, width = 512, height = 512) {
  ...
  entities.forEach((entity) => {
    if (hasComponent(entity, ObjectComponent)) {
      ...
      // Replace all materials with depthMaterial
...
globalThis.createHeightmap = createHeightmap
