/**
 * IR Engine P2P Signaling Worker
 * Based on p2pcf by Greg Fodor (https://github.com/gfodor/p2pcf)
 * 
 * This worker provides WebRTC signaling via Cloudflare Workers and R2 storage
 */
...
async function handleGet(request, env) {
  ...
  return new Response(
    `<html><body style="font-size: 24px; padding: 18px; font-family: Arial, sans-serif"">IR Engine P2P Signaling Worker<br/><div style="line-height: 28px; margin-top: 8px; font-size: 0.8em">${
      hasStore
        ? '&#128077; R2 bucket is configured properly, ready to serve.'
        : '&#10060; Couldn\'t find a configured R2 bucket.<br/>Make sure you created a bucket and connected the worker to it.'
    }</div></body></html>`,
    {
      headers: {
...
async function handlePost(request, env, context) {
  ...
  if (payload.d && payload.p) {
    ...
    if (shouldSave) {
      ...
      // First search for an existing one
      for (let i = 0; i < entries.length; i++) {
        if (entries[i] === null) continue
      ...
      new Promise(res => {
        setTimeout(async () => {
          ...
          if (
            ...
            // Vacuum
            await store.put(`rooms/${roomId}/next_vacuum`, `${now + 30 * 1000}`) // 30 second room vacuum interval
