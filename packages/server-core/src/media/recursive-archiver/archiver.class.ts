...
const archive = async (app: Application, projectName: string, params?: ArchiverParams): Promise<string> => {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    try {
      ...
      // In TypeScript 5.8.3, we need to ensure Buffer is properly handled
      // Convert Buffer to Uint8Array for compatibility with JSZip
      const bufferData = blobPromise instanceof Buffer ? new Uint8Array(blobPromise) : blobPromise
      zip.file(dir, bufferData)
    } catch (err) {
