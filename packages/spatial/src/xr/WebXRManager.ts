...
function ifCase(session, attributes, framebufferScaleFactor, gl, xrRendererState, newRenderTarget, renderer) {
  const layerInit = {
...
/** @todo Rename this function */
function elseCase(attributes, gl, framebufferScaleFactor, scope, session, xrRendererState, newRenderTarget, renderer) {
...
function getScopeFunctionSetSession(xrRendererState, renderer, onSessionEnd, scope, animation) {
  return async function (session: XRSession, framebufferScaleFactor = 1) {
    ...
    if (session.renderState.layers === undefined || renderer.capabilities.isWebGL2 === false) {
      ifCase(session, attributes, framebufferScaleFactor, gl, xrRendererState, newRenderTarget, renderer)
    } else {
      elseCase(attributes, gl, framebufferScaleFactor, scope, session, xrRendererState, newRenderTarget, renderer)
    }
...
export function createWebXRManager(renderer: WebGLRenderer) {
  ...
  scope.setSession = getScopeFunctionSetSession(xrRendererState, renderer, onSessionEnd, scope, animation)
