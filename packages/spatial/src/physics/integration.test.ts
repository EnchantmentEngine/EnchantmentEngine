...
describe('Integration : PhysicsSystem + PhysicsPreTransformSystem + TransformSystem', () => {
  describe('Physics Driven Transformations', () => {
    ...
    beforeEach(async () => {
      ...
      physicsWorld = Physics.createWorld(physicsWorldEntity, { gravity: new Vector3(0, 0, 0), substeps: 1 })
    ...
    /** @todo this is failing because the impulse seems to be moving it half as far as it should, not sure why this has regressed */
    it.skip("should commit pose modifications generated by the Physics into the entity's TransformComponent data", async () => {
      ...
      setComponent(testEntity, RigidBodyComponent, { type: BodyTypes.Dynamic, gravityScale: 0 })
      setComponent(testEntity, ColliderComponent, { mass: 1 })
      await flushAll()
      TransformComponent.dirty[testEntity] = 0
      const Expected: Result = {
        before: {
          physicsSystem: new Vector3(0, 0, 0),
          physicsPreTransformSystem: new Vector3(0, 0, 0),
          transformSystem: new Vector3(0, 0, 0)
        },
        after: {
          physicsSystem: new Vector3(1 / 60, 2 / 60, 3 / 60),
          physicsPreTransformSystem: new Vector3(1 / 60, 2 / 60, 3 / 60),
          transformSystem: new Vector3(1 / 60, 2 / 60, 3 / 60)
        }
      }
      ...
      result.physicsSystem = getComponent(testEntity, RigidBodyComponent).position
  ...
  describe('Transform Overrides (aka teleportation)', () => {
    ...
    it('should move the RigidBody+Collider of an entity relative to its parent', async () => {
      ...
      getComponent(testEntity, TransformComponent).position.addScalar(ChangeAmount)
      // .. Phase 1
      execute.physicsSystem()
      // .. Phase 2
      getMutableState(ECSState).simulationTime.set(
    ...
    it('should allow moving the RigidBody of an entity separately from its Transform, and the movement should be relative to its parent', async () => {
      ...
      // Run and Check the results
      getComponent(testEntity, RigidBodyComponent).position.addScalar(ChangeAmount)
      // .. Phase 1
      execute.physicsSystem()
      // .. Phase 2
      getMutableState(ECSState).simulationTime.set(
    ...
    describe('should apply parent.transform overrides to all the child entities contained in its EntityTree ...', () => {
      ...
      it('... rotation', async () => {
        ...
        // Run and Check the results
        getComponent(testEntity, TransformComponent).rotation.setFromAxisAngle(Axis.Y, ChangeAmount)
        // .. Phase 1
        execute.physicsSystem()
        // .. Phase 2
        getMutableState(ECSState).simulationTime.set(
      ...
      it('... scale', async () => {
        ...
        // Run and Check the results
        getComponent(testEntity, TransformComponent).scale.copy(Initial.scale)
        getComponent(testEntity, TransformComponent).scale.addScalar(ChangeAmount)
        // .. Phase 1
        execute.physicsSystem()
        // .. Phase 2
        getMutableState(ECSState).simulationTime.set(
    ...
    describe("should apply scene.transform modifications to all entities contained in the scene's hierarchy", () => {
      ...
      it('... rotation', async () => {
        ...
        // Run and Check the results
        // .. Phase 0
        getComponent(physicsWorldEntity, TransformComponent).rotation.setFromAxisAngle(Axis.Y, ChangeAmount)
        {
          /** @todo This is a BUG. See TransformComponent.getMatrixRelativeToScene */
          TransformComponent.computeTransformMatrix(physicsWorldEntity)
          TransformComponent.computeTransformMatrix(parentEntity)
      ...
      it('... scale', async () => {
        ...
        // Run and Check the results
        // .. Phase 0
        getComponent(physicsWorldEntity, TransformComponent).scale.addScalar(ChangeAmount)
        {
          /** @todo This is a BUG. See TransformComponent.getMatrixRelativeToScene */
          TransformComponent.computeTransformMatrix(physicsWorldEntity)
          TransformComponent.computeTransformMatrix(parentEntity)
