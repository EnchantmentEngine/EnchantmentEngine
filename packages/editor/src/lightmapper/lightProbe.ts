...
function readTexel(readLightProbe: () => Generator<ProbeDataReport>, probePixelAreaLookup: number[]) {
  let r = 0,
    g = 0,
    b = 0,
    totalDivider = 0

  for (const { rgbaData: probeData, rowPixelStride, probeBox: box, originX, originY } of readLightProbe()) {
...
export async function withLightProbe(
  ...
  taskCallback: (renderLightProbeBatch: ProbeBatcher, debugLightProbeTexture: THREE.Texture) => Promise<void>
) {
  ...
  const renderLightProbeBatch: ProbeBatcher = function* renderLightProbeBatch(gl, lightScene, texelIterator) {
    ...
    for (let batchItem = 0; batchItem < PROBE_BATCH_COUNT; batchItem += 1) {
      ...
      if (!texelResult.done && texelResult.value) {
        const { texelIndex, originalMesh, originalBuffer, faceIndex, pU, pV } = texelResult.value
        ...
        setBlendedNormal(tmpNormal, origNormalArray, origIndexArray, faceVertexBase, pU, pV)
        ...
        setUpProbeSide(probeCam, originalMesh, tmpOrigin, tmpNormal, tmpU, -1)
        probeTarget.viewport.set(probeTargetSize, batchOffsetY, probeTargetSize, probeTargetSize)
        probeTarget.scissor.set(probeTargetSize, batchOffsetY + halfSize, probeTargetSize, halfSize)
        gl.setRenderTarget(probeTarget) // propagate latest target params
        gl.render(lightScene, probeCam)

        setUpProbeSide(probeCam, originalMesh, tmpOrigin, tmpNormal, tmpV, 1)
        probeTarget.viewport.set(probeTargetSize * 2, batchOffsetY, probeTargetSize, probeTargetSize)
        probeTarget.scissor.set(probeTargetSize * 2, batchOffsetY + halfSize, probeTargetSize, halfSize)
        gl.setRenderTarget(probeTarget) // propagate latest target params
        gl.render(lightScene, probeCam)

        setUpProbeSide(probeCam, originalMesh, tmpOrigin, tmpNormal, tmpV, -1)
        probeTarget.viewport.set(probeTargetSize * 3, batchOffsetY, probeTargetSize, probeTargetSize)
        probeTarget.scissor.set(probeTargetSize * 3, batchOffsetY + halfSize, probeTargetSize, halfSize)
        gl.setRenderTarget(probeTarget) // propagate latest target params
        gl.render(lightScene, probeCam)
      } else {
    ...
    gl.readRenderTargetPixels(probeTarget, 0, 0, targetWidth, targetHeight, probeData)
    ...
    for (let batchItem = 0; batchItem < PROBE_BATCH_COUNT; batchItem += 1) {
      ...
      const probePartsReporter = function* () {
        ...
        tmpProbeBox.set(0, batchOffsetY + probeTargetSize, probeTargetSize, probeTargetSize)
        ...
        tmpProbeBox.set(probeTargetSize, batchOffsetY + halfSize, probeTargetSize, halfSize)
        probeDataReport.originX = 0
        probeDataReport.originX = halfSize
        yield probeDataReport

        tmpProbeBox.set(probeTargetSize * 2, batchOffsetY + halfSize, probeTargetSize, halfSize)
        probeDataReport.originX = 0
        probeDataReport.originX = halfSize
        yield probeDataReport

        tmpProbeBox.set(probeTargetSize * 3, batchOffsetY + halfSize, probeTargetSize, halfSize)
        probeDataReport.originX = 0
        probeDataReport.originX = halfSize
        yield probeDataReport
      }
