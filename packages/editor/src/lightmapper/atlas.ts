...
function getTexelInfo(atlasMap: AtlasMap, texelIndex: number): ProbeTexel | null {
...
function getInputItems(sceneItems: Generator<THREE.Object3D, void, unknown>) {
  ...
  for (const mesh of sceneItems) {
    ...
    const atlasPosAttr = new THREE.Float32BufferAttribute(faceVertexCount * 3, 3)
    const atlasUV2Attr = new THREE.Float32BufferAttribute(faceVertexCount * 2, 2)
    const atlasFaceInfoAttr = new THREE.Float32BufferAttribute(faceVertexCount * 4, 4)
    ...
    for (let faceVertexIndex = 0; faceVertexIndex < faceVertexCount; faceVertexIndex += 1) {
