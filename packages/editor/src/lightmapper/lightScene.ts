...
import { traversalStateIsReadOnly, traverseSceneItems, Workbench } from './workbench'
...
function materialIsSupported(material: THREE.Material): material is SupportedMaterial {
  return (
...
const ORIGINAL_MATERIAL_KEY = Symbol('lightmap baker: stashed original material')
type UserDataStore<T extends symbol, V> = Record<T, V | undefined>

export async function withLightScene(workbench: Workbench, taskCallback: () => Promise<void>) {
  // prepare the scene for baking
  const { aoMode, emissiveMultiplier, bounceMultiplier, lightScene, irradiance } = workbench

  // process relevant meshes
  const meshCleanupList: THREE.Mesh[] = []
  const suppressedCleanupList: THREE.Object3D[] = []

  for (const object of traverseSceneItems(lightScene, false, (ignoredObject) => {
    ...
    const materialList: (THREE.Material | null)[] = Array.isArray(mesh.material) ? mesh.material : [mesh.material]
    ...
    const stagingMaterialList = materialList.map((material) => {
      ...
      if (aoMode) {
        if (material.aoMap && material.aoMap !== irradiance) {
          throw new Error('do not set your own AO map manually on baked scene meshes')
        }
      } else {
        if (material.lightMap && material.lightMap !== irradiance) {
          throw new Error('do not set your own light map manually on baked scene meshes')
        }
      }
      ...
      if (!aoMode) {
        ...
        stagingMaterial.emissiveIntensity = material.emissiveIntensity * emissiveMultiplier
      ...
      if (traversalStateIsReadOnly) {
        ...
        if (!aoMode) {
          ...
          stagingMaterial.lightMapIntensity = material.lightMapIntensity * bounceMultiplier
        }
      } else {
    ...
    ;(mesh.userData as UserDataStore<typeof ORIGINAL_MATERIAL_KEY, THREE.Material[] | THREE.Material>)[
      ORIGINAL_MATERIAL_KEY
    ] = mesh.material

    // assign updated list or single material
    mesh.material = Array.isArray(mesh.material) ? stagingMaterialList : stagingMaterialList[0]
  ...
  try {
    ...
    meshCleanupList.forEach((mesh) => {
      ...
      if (finishedTask) {
        const materialList: (THREE.Material | null)[] = Array.isArray(origMaterialValue)
          ? origMaterialValue
          : [origMaterialValue]
