...
function findVertex(
  ...
  for (let vStart = 0; vStart < vertexIndex; vStart += 1) {
    ...
    if (tmpVert2.equals(tmpVert) && tmpNormal2.equals(tmpNormal) && gi === gi2) {
      return vStart
    }
  }
...
function guessOrthogonalOrigin(indexArray: ArrayLike<number>, vStart: number, posArray: ArrayLike<number>): number {
  let minAbsDot = 1
  let minI = 0
...
interface AutoUVBox extends PotpackBox {
...
export function computeAutoUV2Layout(
  ...
  for (const mesh of items) {
    ...
    const uv2Attr = new THREE.Float32BufferAttribute((2 * posArray.length) / 3, 2)
  ...
  for (const layoutBox of layoutBoxes) {
    const { uAxis, vAxis, posArray, posIndices, posLocalX, posLocalY } = layoutBox
  ...
  if ((initialWidth && layoutWidth > initialWidth) || (initialHeight && layoutHeight > initialHeight)) {
    throw new Error(
  ...
  for (const layoutBox of layoutBoxes) {
    ...
    for (let i = 0; i < posIndices.length; i += 1) {
      uv2Attr.setXY(posIndices[i], (ix + posLocalX[i] * iw) / finalWidth, (iy + posLocalY[i] * ih) / finalHeight)
    }
  }
